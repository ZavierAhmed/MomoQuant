        ...
        StrategyDataRequirementDto? requirement)
    {
        return code switch
        {
            ...
            StrategyCode.MomoAdaptiveMultiTimeframeTrendBreakout => new StrategyContent(
                ...
                ApproximationNotes: "Deterministic multi-timeframe trend-breakout. Only closed HTF candles with CloseTimeUtc <= evaluation time are visible. No AI/exchange order placement.",
                ImplementationNotes: requirement?.Notes),
            StrategyCode.MomoVolatilityRangeReversion => new StrategyContent(
                HowItWorks: "Builds a prior lookback range, requires ATR-normalized range width, EMA separation/slope flatness, ATR14/ATR100 band, then enters on boundary sweep with inside-range reclaim, RSI extremity and wick rejection.",
                EntryLogic: "Long: probe below range low then close back inside [rangeLow, rangeHigh] with RSI <= oversold and minimum lower-wick percent. Short is the mirror at the upper boundary. Target is the range midpoint when targetMode=RangeMidpoint.",
                ...
                NoTradeConditions: "InsufficientData, RangeTooNarrow, RangeTooWide, TrendFilterFailed, VolatilityTooLow, VolatilityTooHigh, NoBoundaryProbe, CloseDidNotReclaim, RsiNotExtreme, WickConfirmationMissing, RewardRiskInsufficient, InvalidStop, InvalidTargetMode, StrengthBelowMinimum, DuplicateSetup.",
